...
在Windows上编译，请安装好相关依赖后，使用如下指令编译即可：

```
cmake -DCMAKE_TOOLCHAIN_FILE=D:\hi_slam_compl\3rdparty\scripts\buildsystems\vcpkg.cmake -DCMAKE_BUILD_TYPE:STRING=RelWithDebInfo -DGTSAM_BUILD_TIMING_ALWAYS=OFF -DGTSAM_BUILD_UNSTABLE=OFF -DGTSAM_UNSTABLE_BUILD_PYTHON=OFF -DGTSAM_USE_SYSTEM_EIGEN=On -DGTSAM_USE_SYSTEM_METIS=On -DGTSAM_INSTALL_CPPUNITLITE=OFF -DGTSAM_BUILD_TYPE_POSTFIXES=OFF -DCMAKE_CXX_STANDARD=11  ..


cmake --build . --config RelWithDebInfo --target ALL_BUILD -j 14
```

参考ports下的portfile.cmake
